 ...
 */
public class HdprintedDesFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        desImage = (CardView) rootView.findViewById(R.id.handprinted_des_image_card);
        desImage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(getActivity(), DisplayActivity.class);
                intent.putExtra(Constants.IMAGE_RES_ID, R.drawable.handprinted_header_480);
                if (Build.VERSION.SDK_INT >= 21) {
                    desImage.setTransitionName(getString(R.string.transition_name));
                    startActivity(intent, ActivityOptionsCompat.makeSceneTransitionAnimation(
                            getActivity(), desImage, getString(R.string.transition_name)).toBundle());
                } else {
